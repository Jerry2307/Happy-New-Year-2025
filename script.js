...
const celebrationSound = new Audio("ThanTaiDen-NguyenPhiHung_4hyfb.mp3"); // Âm thanh chúc mừng
...
spinCountText.innerHTML = `<p>Số lần đã quay: ${spinCount}/1</p>`;
...
if (spinCount >= 2) {
  if (lastSpinTime) {
    ...
    if (timeDifference < oneYear) {
      ...
      text.innerHTML = `<p>Em đã quay đủ 1 lần. Vui lòng thử lại sau ${remainingDays} ngày!</p>`;
    } else {
...
spinBtn.addEventListener("click", () => {
  if (spinCount >= 1) {
    alert("Em đã quay đủ 1 lần. Năm sau quay tiếp nhe hehe!");
    return;
  }
